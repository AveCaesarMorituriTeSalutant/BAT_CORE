...
def help():
    ...
    {g}[+] Good headers. Already used in your website. Good job!{reset}
    {w}[+] Good headers. A recommendation to apply it{reset}
    {m}[-] Bad headers. A recommendation to remove it{reset}\n'''.format(g=Fore.GREEN, w=Fore.BLUE, m=Fore.MAGENTA, reset=Style.RESET_ALL))
...
def print_line(message, level=1, category = None, title = None, status=False):
    ...
    if title:
        print(' '*4*level + title + ': ' + message)
    else:
        print(' '*4*level + color + Style.BRIGHT + pre + message)
